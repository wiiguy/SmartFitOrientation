...
class SmartFitOrientationPlugin(Extension):
    def __init__(self):
        super().__init__()
        ...
        CuraApplication.getInstance().getPreferences().addPreference("SmartFitOrientation/do_auto_orientation", False)
    ...
    def _onPreferencesChanged(self, name: str) -> None:
        if name != "SmartFitOrientation/do_auto_orientation":
            return
        self._do_auto_orientation = CuraApplication.getInstance().getPreferences().getValue("SmartFitOrientation/do_auto_orientation")
    ...
    def checkQueuedNodes(self):
        for node in self._check_node_queue:
            ...
            auto_orient_message = Message(i18n_catalog.i18nc("@info:status", "Auto-Calculating the optimal orientation because auto orientation is enabled"), 0,
                                    False, -1, title=i18n_catalog.i18nc("@title", "SmartFit Orientation"))
    ...
    def doAutoOrientation(self, extended_mode):
        ...
        if len(selected_nodes) == 0:
            self._message = Message(i18n_catalog.i18nc("@info:status", "No objects selected to orient. Please select one or more objects and try again."), title = i18n_catalog.i18nc("@title", "SmartFit Orientation"))
            self._message.show()
            return

        message = Message(i18n_catalog.i18nc("@info:status", "Calculating the optimal orientation..."), 0, False, -1, title = i18n_catalog.i18nc("@title", "SmartFit Orientation"))
        message.show()
    ...
    def _onFinished(self, job):
        ...
        if job.getMessage() is not None:
            job.getMessage().hide()
            skipped_nodes = job.getSkippedNodeCount()
            if skipped_nodes == 0:
                status_message = i18n_catalog.i18nc("@info:status", "All selected objects have been oriented.")
            else:
                status_message = i18n_catalog.i18nc(
                    "@info:status",
                    "Some selected objects could not be automatically oriented to fit the build volume without resizing."
                )
            self._message = Message(status_message, title=i18n_catalog.i18nc("@title", "SmartFit Orientation"))
            self._message.show()
