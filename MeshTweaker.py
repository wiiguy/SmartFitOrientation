# -*- coding: utf-8 -*-
# MeshTweaker / STL-Tweaker algorithm by Christoph Schranz (https://github.com/ChristophSchranz/Tweaker-3).
# Parameters from his optimization repos (Tweaker-3_optimize-using-ea). Used under LGPL-3.0.
import os
import re
import math
...
class Tweak:
    ...
    def __init__(self, content, extended_mode=False, verbose=True, show_progress=False,
        ...
        # Save raw vertices for build volume fit checking before any preprocessing
        if build_volume is not None:
            self._original_vertices = np.array(content, dtype=np.float64)
        ...
        # If build volume is known, prioritise orientations where the model fits.
        # Orientations that fit are kept at the front (still sorted by unprintability);
        # orientations that don't fit are moved to the back as a fallback.
        if self.build_volume is not None and hasattr(self, '_original_vertices'):
            _Logger.log('d',
                'SmartFitOrientation: checking {} orientations against build volume '
                '{:.1f} x {:.1f} x {:.1f} mm (W x D x H)'.format(
                    len(best_results), *self.build_volume))
            # Compute fit flag once per orientation to avoid double work and double logging
            fit_flags = [self._fits_in_build_volume(r[0]) for r in best_results]
            fits    = [r for r, f in zip(best_results, fit_flags) if f]
            no_fits = [r for r, f in zip(best_results, fit_flags) if not f]
            best_results = fits + no_fits

            if fits:
                sel = fits[0]
                _Logger.log('i',
                    'SmartFitOrientation: {}/{} orientations fit the build volume. '
                    'Selected orient=[{:.3f},{:.3f},{:.3f}] unprintability={:.4g}'.format(
                        len(fits), len(best_results),
                        sel[0][0], sel[0][1], sel[0][2], sel[4]))
            else:
                _Logger.log('w',
                    'SmartFitOrientation: NO orientation fits the build volume '
                    '{:.1f}x{:.1f}x{:.1f}. '
                    'Falling back to best overhang orientation.'.format(*self.build_volume))
    ...
    def _fits_in_build_volume(self, orientation):
        """Return True if the model fits inside the printer's build volume
        when placed with 'orientation' as the downward-facing direction.

        Uses direct vertex projection so no coordinate-system conversion is
        needed — both orientation and _original_vertices live in the same
        world space (Cura Y-up).

        For the footprint check we sample all in-plane rotations (0–179°) in
        1° steps, mirroring the fact that Cura lets the user freely spin the
        model on the build plate.  The orientation passes if ANY rotation angle
        produces a footprint AABB that fits within the build plate.
        """
        bw, bd, bh = self.build_volume

        n = np.array(orientation, dtype=np.float64)
        n_len = np.linalg.norm(n)
        if n_len < 1e-10:
            _Logger.log('d', 'SmartFitOrientation [fit]: orientation degenerate, skipping')
            return True
        n /= n_len

        # ── Height check ────────────────────────────────────────────────────
        proj = np.dot(self._original_vertices, n)
        height = float(np.max(proj) - np.min(proj))
        if height > bh:
            _Logger.log('d',
                'SmartFitOrientation [fit]: orient=[{:.3f},{:.3f},{:.3f}]  '
                'height={:.1f} > limit={:.1f}  → DOES NOT FIT (too tall)'.format(
                    n[0], n[1], n[2], height, bh))
            return False

        # ── Footprint check with full in-plane rotation sweep ───────────────
        # Build an arbitrary orthonormal basis for the footprint plane
        ref = np.array([1.0, 0.0, 0.0]) if abs(n[0]) < 0.9 else np.array([0.0, 1.0, 0.0])
        u = ref - np.dot(ref, n) * n
        u /= np.linalg.norm(u)
        w = np.cross(n, u)

        # Project all vertices onto the footprint plane once
        pu = np.dot(self._original_vertices, u)  # shape (N,)
        pw = np.dot(self._original_vertices, w)  # shape (N,)
        ...
        if fits:
            ...
            best_fp_max  = best_fit_fp_max
            best_angle   = best_fit_angle

        _Logger.log('d',
            'SmartFitOrientation [fit]: orient=[{:.3f},{:.3f},{:.3f}]  '
            'height={:.1f}/{:.1f}  best_footprint={:.1f}x{:.1f}@{:.0f}° vs {:.1f}x{:.1f}  → {}'.format(
                n[0], n[1], n[2],
                height, bh,
                best_fp_min, best_fp_max, best_angle,
                bf_min, bf_max,
